...
class MostrarTablero():
    ...
    def __init__(self, board):
        ...
        py.init()
        self.win = py.display.set_mode((self.dim, self.dim2))
        py.display.set_caption("Code Breaker")
        # Create a surface for the chessboard itself
        self.chessBoard = py.image.load("Image\\tablero.jpg").convert_alpha()
    ...
    def update_screen(self):
        ...
        red = (200, 0, 0)
        bright_red = (255, 0, 0)
        self.button("Retroceder", 180, 520, 160, 50, red, bright_red, 4)
        py.display.update()        
        self.clock.tick(100)
...
# From here is only UI stuff to display menus and buttons ect.
    ...
    def main_menu(self):
        ...
        while self.intro:
            ...
            self.win.fill((0, 0, 0))
            largeText = py.font.SysFont("cooperblack", 60)
            TextSurf, TextRect = self.text_objects("Code Breaker", largeText)
            TextRect.center = (250, 100)                    
            self.win.blit(TextSurf, TextRect)
            mediumText = py.font.SysFont("cooperblack", 30)
            TextSurf, TextRect = self.text_objects("Elige el color:", mediumText)
            TextRect.center = (130, 240)
            self.win.blit(TextSurf, TextRect)
            #ancho/altura/tamaño del boton(vertical)/tamaño del boton(horizontal)
            self.button("Blanco", 210, 300, 100, 50, blue, bright_blue, 1)
            self.button("Negro", 210, 370, 100, 50, blue, bright_blue, 2)
            self.button("Salir", 210, 450, 100, 50, red, bright_red, 3)
    ...
    def game_over_menu(self):
        ...
        while self.game_over:
            ...
            self.win.fill((0, 0, 0))
            largeText = py.font.SysFont("cooperblack", 40)
            mediumText = py.font.SysFont("cooperblack", 30)
            TextSurfMian, TextRectMain = self.text_objects("El juego se terminó", largeText)
            TextRectMain.center = ((self.dim / 2), (self.dim / 2 - 100))
            self.win.blit(TextSurfMian, TextRectMain)

            if self.board.turn == chess.WHITE:
                TextSurf, TextRect = self.text_objects("!Negro Ganó¡", mediumText)
            else:
                TextSurf, TextRect = self.text_objects("!Blanco Ganó¡", mediumText)

            TextRect.center = ((self.dim / 2), (self.dim2 / 2)-20)
            self.win.blit(TextSurf, TextRect)

            self.button("Jugar de nuevo", 160, 400, 200, 50, blue, bright_blue, 4)
            self.button("Salir", 210, 500, 100, 50, red, bright_red, 3)
    ...
    def button(self, msg, x, y, w, h, ic, ac, action=None):
        ...
        smallText = py.font.SysFont("cooperblack", 20)
...
class Piece():
    ...
    def __init__(self, name, color):
        ...
        if (self.color == "W") or (self.color == "White"):
            if self.name != "Knight":
                self.pieceSurface = py.image.load("Image\\w"+self.name.upper()[0]+".jpg")
            else:
                self.pieceSurface = py.image.load("Image\wN.jpg")
        else:
            if self.name != "Knight":
                self.pieceSurface = py.image.load("Image\\b" + self.name.upper()[0] + ".jpg")
            else:
                self.pieceSurface = py.image.load("Image\\bN.jpg")
